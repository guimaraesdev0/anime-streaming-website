...
$descricao = filter_input(INPUT_POST, 'descricao', FILTER_SANITIZE_STRING);
    ...
    if ($row_usuariov['nome'] == $nome) {
        ...
        $result_usuario = "UPDATE usuarios SET descricao='$descricao', modified=NOW() WHERE auth='$auth'";
        $resultado_usuario = mysqli_query($conn, $result_usuario);
        header("Location: editar.php");
    }else {
        $result_usuario = "UPDATE usuarios SET email='$email', nome='$nome', descricao='$descricao', modified=NOW() WHERE auth='$auth'";
        $resultado_usuario = mysqli_query($conn, $result_usuario);
